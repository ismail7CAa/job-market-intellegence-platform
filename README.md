...
- **Search behaves like a backend engine, not a keyword demo.** `/jobs/search` supports pagination, sorting, company/role/salary/employment filters, ESCO-expanded German-English matching, relevance scores, and match reasons for each result.
- **Missing salaries are estimated explicitly.** Jobs without listed salary use role type, location, occupation group, and experience-level peers for salary estimates, and every estimate is labeled with `salary_type=estimated`, `salary_is_estimated=true`, confidence, and estimation basis.
...
- Startup validation fails when deployment config is unsafe: wildcard CORS with `DEBUG=false`, enabled ingestion without `INGESTION_API_TOKEN`, unsupported `DATABASE_URL`, weak rate-limit settings, or a missing `PRODUCTION_DATA_PATH`.
- Public requests are rate-limited per client IP with `RATE_LIMIT_REQUESTS` and `RATE_LIMIT_WINDOW_SECONDS`.
- Request logging records method, path, status, client IP, request ID, and duration without logging query payloads.
- `X-Request-ID` is accepted from callers or generated by the API, returned on every response, and included in error logs.
- Set `LOG_JSON=true` in deployment for structured JSON logs; local development defaults to readable text logs.
- Error responses use one contract: `{"error": "...", "message": "...", "details": {}}`.
- Search quality regressions protect German/English occupation synonyms, exact-title ranking, salary-type labeling, and expired-listing exclusion.
