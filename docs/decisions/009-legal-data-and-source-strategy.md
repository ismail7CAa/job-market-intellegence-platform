...
### 8. Deployment config needed fail-fast validation

Runtime guardrails are useful, but they are not enough if the application can start with an unsafe public configuration. Bad deployment settings should fail during startup rather than become a public security issue.

Problems:

- wildcard CORS with `DEBUG=false` would expose the API too broadly
- enabling ingestion without an admin token would make the protected route impossible to secure correctly
- invalid or unsupported database URLs would fail later in less obvious places
- overly permissive rate-limit settings would weaken the public backend guardrail
- missing production data would make readiness and search behavior fail after deployment

Decision:

- Validate `DATABASE_URL` during settings startup and allow only SQLite or PostgreSQL drivers for this backend stage.
- Reject `CORS_ALLOW_ORIGINS=["*"]` when `DEBUG=false`.
- Add `INGESTION_ENABLED` and require `INGESTION_API_TOKEN` whenever ingestion is enabled outside debug mode.
- Reject non-positive rate-limit values everywhere and very permissive deployment values when `DEBUG=false`.
- Require `PRODUCTION_DATA_PATH` to point to an existing file when `DEBUG=false`.
- Add regression tests for these deployment safety rules.

Not chosen:

- Let deployment scripts catch these mistakes. Startup validation is closer to the app contract and protects every runtime path, including local Docker, EC2, and future schedulers.
- Permit every SQLAlchemy-supported driver. The repository and migrations are currently engineered around SQLite for local development and PostgreSQL for deployment.

### 9. Ingestion needed one orchestration path
...
### 10. Ingestion batches needed persistence

The ingestion service returned a useful summary, but that summary disappeared after the API response or CLI command finished. A serious backend needs an audit trail of ingestion attempts, especially when providers, scheduled jobs, and source-governance failures are involved.

Problems:

- successful and failed ingestion runs were visible only in logs or immediate responses
- operators could not query recent batch status from the database
- provider refresh history needed counts and error messages for debugging
- future schedulers need a durable place to record batch lifecycle state

Decision:

- Add an `ingestion_batches` table.
- Store source, status, fetched count, saved count, expired count, started timestamp, finished timestamp, and error message.
- Add `IngestionBatchRepository` for batch lifecycle writes.
- Have `IngestionService` create a running batch row, complete it on success, and mark it failed on source-policy or validation/runtime errors.
- Keep dry runs non-persistent so they remain safe validation checks.

### 11. Expired listings needed explicit state
...
### 12. Similar jobs needed persisted matching signals
...
### 13. Database changes needed real migrations
...
### 14. Local SQLite schemas drifted from the SQLAlchemy model
